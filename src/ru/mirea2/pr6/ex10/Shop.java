...
public class Shop {
    ...
    public Computer findComputer(String stComplectation) {
        ...
        for (int i = 0; i < shop_goods.size(); i++) {
            if (shop_goods.get(i).name() == ComputerNames.TOP && stComplectation.equals(ComputerNames.TOP.getStTitle())) {
                if (shop_goods.get(i).price() > tmp.price()) {
                    tmp = shop_goods.get(i);
                }
            } else if (shop_goods.get(i).name() == ComputerNames.MIDDLE && stComplectation.equals(ComputerNames.MIDDLE.getStTitle())) {
                if (shop_goods.get(i).price() <= tmp.price()) {
                    tmp = shop_goods.get(i);
                }
            } else if (shop_goods.get(i).name() == ComputerNames.LOW && stComplectation.equals(ComputerNames.LOW.getStTitle())) {
                if (shop_goods.get(i).price() < tmp.price()) {
                    tmp = shop_goods.get(i);
                }
            }
